...
def train(config: RunConfig):
    ...
    for running_class_index, class_name in IDX2NAME.items():
        ...
        img_dir_path = f"img/cartoon_A_dog-latents*1000_rgrad_gscale1_r_grad0.01_{class_name}/train"
                        ...
                        for i, t in enumerate(scheduler.timesteps):
                            ...
                            if i < grad_update_step:  # update only partial
                                with torch.enable_grad():
                                    ...
                                    x_in_temp  = torch.nn.functional.interpolate(image, size=224)
                                    out = classification_model(x_in_temp)
                                    loss_ce = criterion(out, torch.LongTensor([class_infer]).cuda())
                                    c_grad = torch.autograd.grad(loss_ce.sum(), guided_latents)[0]
                                    #print("r_grad is :  ",r_grad.mean())
                                    guided_latents = guided_latents - (r_grad + c_grad)
                                    '''
                                    guided_latents = guided_latents - r_grad
                                    #print("guided_latent-latents :",torch.mean(guided_latents-latents))

                                with torch.no_grad():
                                    latents = latents + 1000*(guided_latents-latents)
                                    latent_model_input = (
...
def evaluate(config: RunConfig):
    ...
    # Create eval dir
    img_dir_path = f"img/cartoon_A_dog-latents*1000_rgrad_gscale1_r_grad0.01_{class_name}/eval"
    if Path(img_dir_path).exists():
