        ...
        for epoch in range(config.num_train_epochs):
            ...
            for step, batch in enumerate(train_dataloader):
                ...
                with accelerator.accumulate(text_encoder):
                    ...
                    vutils.save_image(best_image,"guidance_map.png",normalize=True, scale_each=True, nrow=int(10))
                    # generate image
...
def evaluate(config: RunConfig):
    ...
    eval_img_list = []
    idx_lst = [282, 282, 282, 436, 436, 436, 636, 636, 636]
    for descriptive_token in tokens_to_try:
        ...
        for seed in range(config.test_size):
            ...
            pred_class = torch.argmax(output).item()
            pred_cls = IDX2NAME[pred_class].split(",")[0]
            if descriptive_token == config.model_initializer_token:
                img_path = (
            ...
            if pred_class == idx:
                correct += 1
            print(f"pred image class: {IDX2NAME[pred_class]}")
