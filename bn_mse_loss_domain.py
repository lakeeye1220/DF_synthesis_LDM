...
def prepare_latents(image, vae, scheduler, timestep, batch_size, num_images_per_prompt, dtype, device, generator=None):
        ...
        if image.shape[1] == 4:
            ...
            if isinstance(generator, list) and len(generator) != batch_size:
                ...
                init_latents = vae.encode(image).latent_dist.sample(generator=generator)
...
def train(config: RunConfig,ref_img:torch.Tensor):
    ...
    for running_class_index, class_name in IDX2NAME.items():
        ...
        with torch.no_grad():
            ...
            scheduler.set_timesteps(config.num_of_SD_inference_steps, device=accelerator.device)
            #timesteps = scheduler.timesteps
            timesteps, num_inference_steps = get_timesteps(scheduler,config.num_of_SD_inference_steps, config.strength, accelerator.device)
            latent_timestep = timesteps[:1].repeat(config.batch_size * config.number_of_prompts)

            di_latents = prepare_latents(
            di_img,
            vae,
            scheduler,
            latent_timestep,
            config.batch_size,
            config.number_of_prompts,
            weight_dtype,
            accelerator.device,
            generator,
            )
        ...
        utils.numpy_to_pil(
            ...
            )[0].save(f"{img_dir_path}/A {prompt_suffix}.jpg",
            "JPEG",
        )
