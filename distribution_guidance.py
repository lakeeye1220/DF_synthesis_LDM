...
def train(config: RunConfig):
    ...
    for running_class_index, class_name in IDX2NAME.items():
        ...
        img_dir_path = f"img/A_dog-latents*100_rgrad_gscale1_r_grad0.01_{class_name}/train"
                        ...
                        for i, t in enumerate(scheduler.timesteps):
                            ...
                            if i < grad_update_step:  # update only partial
                                with torch.enable_grad():
                                    ...
                                    r_grad = torch.autograd.grad(0.001*loss_r_feature.sum(), guided_latents)[0]
                                    '''
                                    guided_latents = guided_latents.detach().requires_grad_(True)
                                    input_latents = 1/0.18215*guided_latents
                                    image = vae.decode(input_latents).sample
                                ...
                                with torch.no_grad():
                                    latents = latents + 100*(guided_latents-latents)
                                    latent_model_input = (
...
def evaluate(config: RunConfig):
    ...
    # Create eval dir
    img_dir_path = f"img/A_dog-latents*100_rgrad_gscale1_r_grad0.01_{class_name}/eval"
    if Path(img_dir_path).exists():
