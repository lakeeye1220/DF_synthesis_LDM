...
def show_cross_attention(pipe:torch.FloatTensor, prompts:List[str] ,attention_store: AttentionStore, res: int, from_where: List[str], select: int = 0, path=""):
    ...
    for i in range(len(tokens)):
        ...
        #plt.colorbar()
        #plt.title("Average Attention Map Heatmap")
        #plt.show()
        image = ptp_utils.text_under_image(image, decoder(int(tokens[i])))
        images.append(image)
    #ptp_utils.view_images(np.stack(images, axis=0),path,"cross_attention_map")
    ptp_utils.view_images(np.stack(images, axis=0), num_rows=1, offset_ratio=0.02, prefix=path,postfix="cross_attention_map")
    

def show_self_attention_comp(attention_store: AttentionStore, res: int, from_where: List[str],
                        max_com=10, select: int = 0,path=""):
    ...
    for i in range(max_com):
        ...
        images.append(image)
    ptp_utils.view_images(np.concatenate(images, axis=1),path,"self_attention_map")

def run_and_display(ldm_stable, prompts, controller, latent=None, run_baseline=False, generator=None,path=""):
    if run_baseline:
    ...
    ptp_utils.view_images(images,path,"generated_image")
    return images, x_t
...
ptp_utils.view_images(images,prefix="",postfix="attention_map")
show_cross_attention(controller, res=16, from_where=("up", "down"))
#show_cross_attention(controller, 24, ["up", "down"])
'''