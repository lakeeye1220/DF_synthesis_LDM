...
import yj_prompt_dataset as prompt_dataset
import yj_utils
from inet_classes import IDX2NAME as IDX2NAME_INET
from transformers import BeitForImageClassification
from yj_config import RunConfig
import pyrallis
import shutil
...
def train(config: RunConfig):
    ...
    if config.classifier == "inet":
        ...
        from yj_classes import IDX2NAME as IDX2NAME_INETR
        IDX2NAME = IDX2NAME_INETR
    else:
        IDX2NAME = classification_model.config.id2label

    #### Train ####
    print(f"Start experiment {exp_identifier}")
    unet, vae, text_encoder, scheduler, tokenizer = yj_utils.prepare_stable(config)
    ...
    yj_utils.freeze_params(params_to_freeze)
    ...
    yj_utils.freeze_params(vae.parameters())
    yj_utils.freeze_params(unet.parameters())
            ...
            for running_class_index, class_name in IDX2NAME.items():
                ...
                for step, batch in enumerate(train_dataloader):
                    ...
                    with accelerator.accumulate(text_encoder):
                        ...
                        image_out = image
                        image = yj_utils.transform_img_tensor(image, config)
                        image = torch.nn.functional.interpolate(image, size=224)
                        if config.classifier =='imagenet_r_art':
...
def evaluate(config: RunConfig):
    ...
    for descriptive_token in tokens_to_try:
        ...
        for seed in range(config.test_size):
            ...
            image = yj_utils.transform_img_tensor(image_out, config)
            image = torch.nn.functional.interpolate(image, size=224)
            output = classification_model(image).logits
            ...
            yj_utils.numpy_to_pil(image_out.permute(0, 2, 3, 1).cpu().detach().numpy())[
                0
            ].save(img_path, "JPEG")
