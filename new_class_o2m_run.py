        ...
        for epoch in range(config.num_train_epochs):
            ...
            for step, batch in enumerate(train_dataloader):
                ...
                with accelerator.accumulate(text_encoder):
                    ...
                    print('class_infer 이게 수정 전 :',class_infer)
                    ...
                    cls_infer = torch.argmax(output).item()
                    print('cls_infer 이게 수정한거:',cls_infer)
                    confidence = pred_probs[:,cls_infer].mean().item()
                    print('pred_probs :',pred_probs)
                    print(f"Current image's confidence score is {confidence}")
                    utils.numpy_to_pil(
                            image_out.permute(0, 2, 3, 1).cpu().detach().numpy()
                        )[0].save(
                            f"img/testtest/answer:{IDX2NAME_cls[CLS2IDX_cls[label_lst[class_infer]]]},but predict:{IDX2NAME_cls[CLS2IDX_cls[label_lst[cls_infer]]]}.jpg",
                            "JPEG",
                        )
                    ...
                    with torch.no_grad():
                        ...
                        utils.numpy_to_pil(
                            ...
                            f"{img_dir_path}/{epoch}_{IDX2NAME_cls[pred_class]}_{classification_loss.detach().item()}.jpg",
                            "JPEG",
                        )
                    ...
                    if accelerator.sync_gradients:
                        ...
                        if total_loss < min_loss:
                            ...
                            if accelerator.is_main_process:
                                print(
                                    f"Saved the new discriminative class token pipeline of {IDX2NAME_cls[class_infer]} to pipeline_{token_path}"
                                )
                                if config.sd_2_1:
...
def evaluate(config: RunConfig):
    ...
    classification_model.load_state_dict(torch.load(config.model_PATH))
    classification_model.eval()
    # classification_model.to(config.device)
    ...
    eval_img_list = []
    idx_lst = [1,1,1,2,2,2,3,3,3]
    # eval = ['tiger cat', 'rabbit', 'orange', 'teddy bear']
    for descriptive_token in tokens_to_try:
        ...
        for seed in range(config.test_size):
            ...
            pred_class = torch.argmax(output).item()
            pred_cls = IDX2NAME_cls[pred_class].split(",")[0]
            if descriptive_token == config.model_initializer_token:
                img_path = (
                ...
                img_path = (
                    f"{img_dir_path}/{exp_identifier}_{IDX2NAME_cls[pred_class]}.jpg"
                )
            ...
            if pred_class == idx:
                correct += 1
            print(f"pred image class: {IDX2NAME_cls[pred_class]}")
