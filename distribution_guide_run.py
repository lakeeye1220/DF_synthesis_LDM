...
def train(config: RunConfig):
    ...
    for running_class_index, class_name in IDX2NAME.items():
        ...
        img_dir_path = f"img/1rgrad_gscale7_r_grad0.1_{class_name}/train"
                        ...
                        for i, t in enumerate(scheduler.timesteps):
                            ...
                            if i < grad_update_step:  # update only partial
                                with torch.enable_grad():
                                    ...
                                    r_grad = torch.autograd.grad(0.1*loss_r_feature.sum(), guided_latents)[0]
...
def evaluate(config: RunConfig):
    ...
    # Create eval dir
    img_dir_path = f"img/1rgrad_gscale7_r_grad0.1_{class_name}/eval"
    if Path(img_dir_path).exists():
