...
def run(args):
    ...
    elif args.arch_name =='resnet34_cartoon_pacs':
        net = torch.hub.load('pytorch/vision:v0.10.0', 'resnet34', pretrained=False)
        net.fc = nn.Linear(512,7)
        net.load_state_dict(torch.load('../Homework3-PACS/cartoon_model_0.617882.pt'))
        net.eval()

    elif args.arch_name =='resnet34_art_pacs':
        net = torch.hub.load('pytorch/vision:v0.10.0', 'resnet34', pretrained=False)
        net.fc = nn.Linear(512,7)
        net.load_state_dict(torch.load('../Homework3-PACS/from_scratch_art_model_0.547348.pt'))
        net.eval()

    elif args.arch_name =='resnet34_sktech_pacs':
        net = torch.hub.load('pytorch/vision:v0.10.0', 'resnet34', pretrained=False)
        net.fc = nn.Linear(512,7)
        net.load_state_dict(torch.load('../Homework3-PACS/sketch_model_0.174087.pt'))
        net.eval()
...
def main():
    ...
    parser.add_argument('--bs', default=14, type=int, help='batch size')
    ...
    parser.add_argument('--arch_name', default='resnet34_sktech_pacs', type=str, help='model name from torchvision or resnet50v15')

    parser.add_argument('--fp16', action='store_true', help='use FP16 for optimization')
    parser.add_argument('--exp_name', type=str, default='sketch', help='where to store experimental data')
    ...
    parser.add_argument('--do_flip', default=True, action='store_true', help='apply flip during model inversion')
    ...
    parser.add_argument('--store_best_images', default='True', help='save best images as separate files')
    parser.add_argument('--class_index', default=0)
